"""
ex06h_nozzle_refined.py
-----------------------
High-Fidelity Nozzle Simulation.
Features:
  1. Order=2 (Second Order Reconstruction).
  2. Stagnation Inlet BC (Fixes P0/T0).
  3. Standardized Output using SimulationDisplay.
"""
import sys
import numpy as np
...
def update_inlet_bc(solver, grid, p0, rho0, inlet_face_indices):
    ...
    # Subsonic Inlet Logic: Fix P0, Extrapolate P_static
...
def run():
    ...
    grid = Grid(mesh)
    print(f"   -> Generated {grid.n_cells} cells")
    ...
    # Find inlet faces
    inlet_group_id = None
    for gid, name in grid.group_names.items():
        if name == "inlet":
            inlet_group_id = gid
            break
    ...
    disp.setup_stats_columns(
        ["Iter", "Time [ms]", "dt", "Resid", "Max(u+c)"], 
        [6, 12, 10, 10, 10]
    )
    
    # --- VARIABLE INITIALIZATION (Fixes NameError) ---
    total_time = 0.0
    target_time = 0.008 # 8ms
    iter_count = 0
    dt = 1e-7 # Initial tiny step
    resid = 1.0
    max_s = 340.0
    
    while total_time < target_time:
        if iter_count > 0:
            update_inlet_bc(solver, grid, p0, rho0, inlet_faces)
    ...
    plt.plot(theory_x, theory_mach, 'k-', linewidth=2, label="1D Theory")
    plt.plot(sim_x, sim_mach, 'r.', markersize=4, alpha=0.5, label="CFD (Order=2)")
    plt.axvline(xt, color='gray', linestyle='--')
    plt.title(f"Nozzle Validation (Order=2)")
