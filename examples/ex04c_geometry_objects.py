...
Goal: Demonstrate the restored Object-Oriented Geometry system.
      Uses LineSegment and Arc to define a Nozzle boundary.
"""
import numpy as np
...
def create_nozzle_curves():
    """ Defines the nozzle using the new Geometry classes. """
    ...
    # 2. Composite Top Wall
    arc1 = geom.Arc((xi, ri-rci), rci, np.pi/2, np.pi/2 - ani)
    # Note: We use evaluate(1.0) to get exact end point of arc
...
def run():
    print("--- Geometry Object Test ---")
    
    curves, xt, rt = create_nozzle_curves()
    sizing = create_sizing_func(xt, rt)
    
    # Ask objects to discretize themselves
    poly_points = []
    for c in curves:
        pts = c.discretize_adaptive(sizing)
        if len(poly_points) > 0:
            poly_points.append(pts[1:]) # Stitching
        else:
    ...
    mesh = generate_unstructured_mesh(
        [full_poly], sizing, h_base=0.008, n_smooth=30
    )
