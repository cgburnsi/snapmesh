...
UPDATED: Robust Boundary Tagging (Handles Corners).
"""
import numpy as np
from snapmesh.elements import Node # Needed for geometric check
...
class Grid:
    def __init__(self, mesh_obj):
        ...
        for i, e in enumerate(sorted_edges):
            ...
            x1, y1 = e.n1.x, e.n1.y
            x2, y2 = e.n2.x, e.n2.y
            self.face_midpoints[i] = [0.5*(x1+x2), 0.5*(y1+y2)]
            self.face_normals[i] = [y2-y1, -(x2-x1)]
            
            # --- ROBUST BOUNDARY TAGGING ---
            c1 = e.n1.constraint
            c2 = e.n2.constraint
            
            boundary_name = None
            
            # Case 1: Perfect Match (Middle of curve)
            if c1 and c2 and c1 == c2:
                boundary_name = c1.name
            
            # Case 2: Mismatch (Likely a Corner)
            elif c1 or c2:
                candidates = []
                if c1: candidates.append(c1)
                if c2 and c2 != c1: candidates.append(c2)
                
                # We check if both nodes snap to the candidate
                for c in candidates:
                    # Check n1
                    d1 = 0.0
                    if e.n1.constraint != c:
                        dummy = Node(-1, e.n1.x, e.n1.y)
                        c.snap(dummy)
                        d1 = np.hypot(dummy.x - e.n1.x, dummy.y - e.n1.y)
                    
                    # Check n2
                    d2 = 0.0
                    if e.n2.constraint != c:
                        dummy = Node(-1, e.n2.x, e.n2.y)
                        c.snap(dummy)
                        d2 = np.hypot(dummy.x - e.n2.x, dummy.y - e.n2.y)
                        
                    # If both are effectively on the curve
                    if d1 < 1e-5 and d2 < 1e-5:
                        boundary_name = c.name
                        break

            if boundary_name:
                self.face_groups[i] = get_group_id(boundary_name)

        # Step B: Face Neighbors
        ...
        for i in range(self.n_faces):
            ...
            if dx * self.face_normals[i, 0] + dy * self.face_normals[i, 1] < 0:
                self.face_normals[i] *= -1.0
                
        print(f"   -> Found Boundary Groups: {self.group_names}")
